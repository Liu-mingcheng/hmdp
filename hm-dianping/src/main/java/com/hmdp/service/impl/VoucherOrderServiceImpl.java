...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    //创建线程任务，秒杀业务需要在当前类初始化之后，就立即执行，所以这里需要用到@PostConstruct注解
    @PostConstruct
    private void init() {
    ...
    private class VoucherOrderHandler implements Runnable {
        ...
        public void run() {
            while (true) {
                try {
                    //1.获取消息队列中的订单信息 xreadgroup group g1 c1 COUNT1 BLOCK 2000 STREAM streams.order >
        ...
        private void handlePendingList() {
            while (true) {
                try {
                    //1.获取pending-list的订单信息 xreadgroup group g1 c1 COUNT1 BLOCK 2000 STREAM streams.order 0
                    List<MapRecord<String, Object, Object>> list = stringRedisTemplate.opsForStream().read(
                            Consumer.from("g1", "c1"),
                    ...
                    //2.判断消息是否获取成功
                    if (list == null || list.isEmpty()) {
                        //如果获取失败，说明pending-list没有异常消息,跳出循环
                        break;
                    }
    ...
    private void handleVoucherOrder(VoucherOrder voucherOrder) {
        ...
        //3.获取锁 无参 有默认值 失败直接返回不等待，超过30秒没释放自动释放
    ...
    //把代理对象放到成员变量里这样子线程可以获取的到，用来执行事务
    ...
    public Result seckillVoucher(Long voucherId) {
        ...
        //无参 有默认值 失败直接返回不等待，超过30秒没释放自动释放
        boolean isLock = lock.tryLock();
        //判断是否获取锁成功
        if (!isLock) {
            //获取锁失败，返回错误或重试
